...
def mul_interval(x, y):
    ...
    value in y."""
    low_x, low_y = lower_bound(x), lower_bound(y)
    up_x, up_y = upper_bound(x), upper_bound(y)

    p1 = low_x * low_y
    p2 = low_x * up_y
    p3 = up_x * low_y
    p4 = up_x * up_y
    left, right = min(p1, p2, p3, p4), max(p1, p2, p3, p4)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert lower_bound(y) > 0 and upper_bound(y) > 0, 'should not divide by an interval that spans zero'
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    def quadratic_helper(t):
        return a*t**2 + b*t + c

    extreme = -b/(2*a)
    l, u, e = map(quadratic_helper, (lower_bound(x), upper_bound(x), extreme))
