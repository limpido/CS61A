...
class Check(object):
    "*** YOUR CODE HERE ***"
    def __init__(self, payable_to, amount, deposited = False):
        self.amount = amount
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits, count = helper_align(skeleton_idx+1, code_idx+1)
            possibilities.append((skel_char + edits, count))
        # Insert
        edits, count = helper_align(skeleton_idx, code_idx+1)
        edits = "+[" + code_char + "]" + edits
        possibilities.append((edits, count + 1))
        # Delete        
        edits, count = helper_align(skeleton_idx+1, code_idx)
