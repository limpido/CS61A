...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def helper(sentence):
        words = split(lower(remove_punctuation(sentence)))
        for word in words:
            if word in topic:
                return True
        return False
...
def wpm(typed, elapsed):
    ...
    min = elapsed / 60
    return len(typed) / TYPICAL_WORD_LENGTH / min
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for w in valid_words:
        ...
        if diff < min_diff:
            word = w
            min_diff = diff
    return user_word if min_diff > limit else word
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    time_diff = [[times[i] - times[i-1] for i in range(1, len(times))] for times in times_per_player]
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    # find the player with min typing time for each word
    min_time_players = [0] * len(all_words(game))
    for i in word_indices:
        min_time = time(game, 0, i)
        min_time_players[i] = 0
        for j in player_indices:
            if time(game, j, i) < min_time:
                min_time = time(game, j, i)
                min_time_players[i] = j

    # match the min time with player's typing time
    res = [[] for player in player_indices]
    times = all_times(game)
    words = all_words(game)
    for i in word_indices:
        player = min_time_players[i]
        res[player].append(words[i])
    return res
    # END PROBLEM 10
